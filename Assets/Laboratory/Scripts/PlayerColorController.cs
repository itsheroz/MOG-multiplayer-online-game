...
/// This class handles the reactive logic flow (Initialization, Data Ready, and Realtime Updates)
...
public abstract class PlayerColorController : MonoBehaviour
{
    ...
    public abstract bool IsMine { get; } // True if this object belongs to the local user.
    public abstract int OwnerID { get; } // The unique network/session ID of the player who owns this object.
    ...
    protected virtual void Start()
    {
        ...
        // 1. Subscribe: Listen for the service to connect (critical for late connections/initial setup).
        GameDataSessionManager.Instance.OnServiceConnected += InitializeWithService;

        // 2. Check: If the Service is already available, start the setup process immediately.
    ...
    protected virtual void OnDestroy()
    {
        ...
        {
            // Unsubscribe all events to prevent memory leaks and unexpected callbacks.
            GameDataSessionManager.Instance.OnServiceConnected -= InitializeWithService;
    ...
    {
        // 1. Subscribe to Realtime Updates (Must be done first to catch immediate changes).
        session.OnPlayerSessionDataChanged += HandleDataChange;
    ...
    // The primary function executed when the session data is 100% ready for reading.
    ...
    private void HandleDataChange(int playerId, string key, object value)
    {
        ...
        // Filter the event to ensure it only applies to this object and the correct property key.
